...
fn convert_config(py: Python, v: ConfigVal) -> PyObject {
    match v {
        ...
        ConfigVal::ListC(v) => v.into_iter()
            .map(|x| convert_config(py, x))
            .collect::<Vec<PyObject>>()
            .to_object(py),
        ConfigVal::DictC(d)=> d.into_iter()
...
fn pythonize(py: Python, extraction: Extraction) -> PyResult<PyObject> {
    let refs: Vec<Vec<String>> = extraction.refs.into_iter().map(|x| match x {
        (a, Some(b)) => vec![a, b],
        (a, None) => vec![a]
    }).collect();
    let sources: Vec<Vec<String>> = extraction.sources.into_iter().map(|(a, b)| vec![a, b]).collect();
...
pub fn py_extract_from_source(source: &str) -> PyResult<PyObject> {
    ...
    to_py_result(extract_from_source(source))
        .and_then(|x| pythonize(py, x))
}

#[pymodule]
fn extractor(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_wrapped(wrap_pyfunction!(py_extract_from_source)).unwrap();
